...
class TestFrs:
    ...
    def test_verdict_rules_strict(self):
        ...
        frs = compute_frs(
            scores=dict.fromkeys(("T1", "T2", "T3", "T4", "L1", "L2", "L3", "L4", "O1", "O2", "O3", "O4"), 5)
        )
        assert frs["score"] == 60
        ...
        frs = compute_frs(
            scores={"T2": 5, "L3": 5, "O1": 5},  # +3+3+3 = 40
    ...
    def test_frs_block_in_signed_report(self, marked):
        results = {"detect": {"verdict": "watermark_detected", "z_score": 13.6, "p_value": 1e-8}}
        frs = compute_frs()
        report = build_finding_report(results, key_id=KEY, frs=frs, sign_secret=KEY)
...
class TestFindingContext:
    def test_cli_institutional_rule_sets_context(self, tmp_path, marked):
        ...
        r = run_cli(["finding", str(inp), "--key", KEY, "--institutional-rule", "Prüfungsordnung §5 Abs. 2"])
    ...
    def test_cli_origin_history_sets_context(self, tmp_path, marked):
        inp = tmp_path / "in.txt"
        inp.write_text(marked, encoding="utf-8")
        r = run_cli(["finding", str(inp), "--key", KEY, "--origin-history", "Entstanden in Klausur vom 03.07."])
        assert r.returncode == 0, r.stderr
        report = json.loads(r.stdout)
        assert report["findings"][0]["context_missing"] is False
    ...
    def test_api_context_dict_sets_context(self, marked):
        ...
        resp = client.post(
            ...
            json={"text": marked, "key_id": "demo-kgw-1", "context": {"institutional_rule": "APO §12"}},
...
class TestDetectApiFields:
    ...
    def test_detect_without_flags_keeps_old_shape(self, marked, monkeypatch):
        resp = self._client(monkeypatch).post(
            "/api/forensics/detect", json={"text": marked, "key_id": KEY}, headers={"X-API-Key": "test-secret"}
        )
        assert resp.status_code == 200, resp.text
        body = resp.json()
...
class TestTuiExposition:
    def test_menu_has_25_and_all_actions_exist(self):
        ...
        for wanted in ("delta-z", "finding", "report-sign", "report-verify", "report-keygen"):
...
class TestDesktopContext:
    def test_build_report_accepts_context(self, tmp_path, marked):
        ...
        report = ctrl.build_report(
            marked, "demo-kgw-1", context={"institutional_rule": "PO §5", "origin_history": "Klausur"}
        )
