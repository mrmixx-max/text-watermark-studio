...
COMMA_TEXT = ("apples, oranges, pears, bananas, grapes, plums, cherries, "
...
FURTHERMORE_TEXT = ("Furthermore, the system remains stable. Furthermore, "
...
def _marked() -> str:
    return mark_greenlist(
        ...
        KEY, vocab=FREQUENT_VOCAB, seed=0)["text"]


class TestHeuristicNeverClaimsWatermark:
    def test_comma_heuristic_is_hints_only(self):
        keys = [{"key_id": "demo-green-1", "family": "greenlist_bias",
                 "is_demo": True}]
    ...
    def test_furthermore_heuristic_is_hints_only(self):
        keys = [{"key_id": "demo-semantic-1", "family": "semantic_pattern",
                 "trigger_phrase": "furthermore", "is_demo": True}]
    ...
    def test_kgw_mark_still_detected(self):
        keys = [{"key_id": "demo-kgw-1", "family": "kgw",
                 "secret": KEY, "gamma": 0.25, "is_demo": True}]
    ...
    def test_kgw_plus_heuristic_kgw_wins(self):
        keys = [
            {"key_id": "demo-kgw-1", "family": "kgw", "secret": KEY,
             "gamma": 0.25, "is_demo": True},
            {"key_id": "demo-semantic-1", "family": "semantic_pattern",
             "trigger_phrase": "furthermore", "is_demo": True},
        ]
        r = ensemble_detect(_marked(), keys)
        assert r["verdict"] == "strong_consistent_signal", r
...
class TestExcludeDemo:
    def test_exclude_demo_filters_keys(self):
        keys = [
            {"key_id": "demo-kgw-1", "family": "kgw", "secret": KEY,
             "gamma": 0.25, "is_demo": True},
            {"key_id": "real-1", "family": "kgw", "secret": "real-secret",
             "gamma": 0.25},
        ]
    ...
    def test_per_key_marks_is_demo(self):
        keys = [{"key_id": "demo-kgw-1", "family": "kgw", "secret": KEY,
                 "gamma": 0.25, "is_demo": True}]
        r = ensemble_detect(_marked(), keys)
...
class TestApiExcludesDemo:
    def test_api_detect_never_watermark_on_heuristic_text(self, monkeypatch):
        ...
        r = client.post("/api/forensics/detect",
                        json={"text": FURTHERMORE_TEXT},
