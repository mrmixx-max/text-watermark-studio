...
def test_api_detect_single_kgw_pass(monkeypatch, tmp_path):
    ...
    assert body["kgw"]["best"]["verdict"] in ("watermark_detected", "redlist_detected")
...
def test_tui_parse_level_context():
    ...
    assert StudioTUI._parse_level_context("file.txt --level bpe --context 8") == ("bpe", 8)
...
def test_tui_provenance_secrets_from_registry(tmp_path, monkeypatch):
    ...
    reg_path.write_text(
        json.dumps(
            {
                "keys": [{"key_id": "real-key", "family": "kgw", "secret": "real-secret", "status": "active"}],
            }
        ),
        encoding="utf-8",
    )
...
def test_tui_kgw_keys_filters_secretless(tmp_path, monkeypatch):
    ...
    reg_path.write_text(
        json.dumps(
            {
                "keys": [
                    {"key_id": "with-secret", "family": "kgw", "secret": "s1", "status": "active"},
