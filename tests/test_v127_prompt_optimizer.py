...
class TestScoring:
    def test_deterministic_scores(self, optimizer):
        ...
        assert [x["avg_score"] for x in r1["ranking"]] == \
               [x["avg_score"] for x in r2["ranking"]]
...
class TestPromote:
    def test_promote_writes_new_version(self, optimizer):
        ...
        optimizer.registry.create_version({
            "id": "rewrite-default", "version": "1.0.0", "channel": "stable",
            "system_prompt": "Rewrite plainly.", "user_template": "{{text}}",
            "provider": None, "model": None, "parameters": {},
        })
    ...
    def test_promote_rejects_no_improvement(self, optimizer):
        optimizer.registry.create_version({
            "id": "rewrite-default", "version": "1.0.0", "channel": "stable",
            "system_prompt": "x", "user_template": "{{text}}",
            "provider": None, "model": None, "parameters": {},
        })
        ...
        with pytest.raises(ValueError, match="no_improvement|guardrail"):
            optimizer.promote("Rewrite the text.", "rewrite-default",
                              candidate_variant="baseline")

    def test_history_lists_all_versions(self, optimizer):
        optimizer.registry.create_version({
            "id": "t", "version": "1.0.0", "channel": "stable",
            "system_prompt": "a", "user_template": "{{text}}",
            "provider": None, "model": None, "parameters": {},
        })
...
class TestRollback:
    def test_rollback_restores_as_new_stable(self, optimizer):
        optimizer.registry.create_version({
            "id": "t", "version": "1.0.0", "channel": "stable",
            "system_prompt": "original prompt", "user_template": "{{text}}",
            "provider": None, "model": None, "parameters": {},
        })
