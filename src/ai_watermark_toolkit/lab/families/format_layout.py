...
class FamilyPlugin(LabFamily):
    ...
    def detect(self, text: str, options: dict | None = None) -> dict:
        return {'family': self.slug, 'supported': True, 'score': 0.75 if '  ' in text or '\t' in text else 0.1, 'notes': ['layout_spacing_demo']}

    def embed(self, text: str, options: dict | None = None) -> dict:
        return {'family': self.slug, 'supported': True, 'text': text.replace(' ', '  ', 1) if ' ' in text else text, 'notes': ['demo_layout_embed']}
