...
class FamilyPlugin(LabFamily):
    ...
    def detect(self, text: str, options: dict | None = None) -> dict:
        ...
        return {'family': self.slug, 'supported': True, 'score': min(0.85, hits * 0.15), 'notes': ['syntactic_pattern_demo']}

    def embed(self, text: str, options: dict | None = None) -> dict:
        return {'family': self.slug, 'supported': True, 'text': text.replace('.', '; therefore.') if '.' in text else text + '; therefore.', 'notes': ['demo_syntactic_embed']}
