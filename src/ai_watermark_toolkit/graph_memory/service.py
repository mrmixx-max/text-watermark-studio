...
class GraphMemoryService:
    ...
    def ingest_fact(
        ...
        return self.add_edge(
            {
                "source": subject.lower().replace(" ", "_"),
    ...
    def neighbors(self, node_id: str):
        g = self._load_graph()
        nbr_edges = [e for e in g["edges"] if e["source"] == node_id or e["target"] == node_id]
    ...
    def query(self, label: str):
        ...
        matches = [
            n
            for n in g["nodes"]
            if label.lower() in n.get("label", "").lower() or label.lower() in n.get("id", "").lower()
        ]
