...
def _register_commands(sub) -> None:
    """Register all subcommand parsers on *sub*."""
    ...
    d.add_argument(
        "--key",
        default=None,
        help="key_id / secret for the keyed KGW test (enables real Z-score detection)",
    )
    d.add_argument(
        "--key-file",
        default=None,
    ...
    d.add_argument(
        ...
        default="word",
        choices=["word", "bpe"],
        help="token level for KGW detection (default word)",
    )
    ...
    em.add_argument(
        ...
        default="word",
        choices=["word", "bpe"],
        help="token level for greenlist marking (default word)",
    )
    ...
    rv = sub.add_parser(
        "report-verify",
        help="verify a signed forensic findings document (exit 0 valid / 1 invalid / 2 usage)",
    )
    ...
    rk = sub.add_parser(
        "report-keygen",
        help="generate an ML-DSA keypair for signing forensic findings (FIPS 204, needs cryptography)",
    )
    ...
    rk.add_argument(
        "--prefix",
        default="mldsa",
        help="file name prefix (default mldsa -> mldsa_private.pem / mldsa_public.pem)",
    )
    ...
    bt.add_argument(
        ...
        default="word",
        choices=["word", "bpe"],
        help="token level for --mode embed (default word)",
    )
    ...
    bt.add_argument(
        ...
        type=float,
        default=None,
        help="greenlist fraction for --mode embed (default: key's gamma or 0.25)",
    )
    ...
    ks = sub.add_parser(
        "kgw-sample",
        help="generate synthetic KGW-bias text and detect it (experimental generation-time bias demo)",
    )
    ...
    dz = sub.add_parser(
        "delta-z",
        help="ΔZ check: measure KGW watermark strength before vs after (removal with receipt)",
    )
    dz.add_argument(
        "before",
        nargs="?",
        help="file with the text BEFORE cleaning/attack (single file when --transform is used)",
    )
    ...
    dz.add_argument(
        "--key",
        default=None,
        help="key_id from data/key_registry.json (must carry a secret) or a raw secret",
    )
    dz.add_argument(
        "--key-file",
        default=None,
    ...
    dz.add_argument(
        "--sign",
        default=None,
        help="HMAC secret: sign the ΔZ result (signed_report) for an auditable document",
    )
    ...
    dz.add_argument("-o", "--output", default=None, help="write the JSON result to a file instead of stdout")
        ...
        "--e-value",
        action="store_true",
        help="also run the anytime-valid e-process (E-Wert-Befund, Klasse C)",
    )
    fi.add_argument(
        "--delta-z",
        metavar="FILE_AFTER",
    ...
    tr.add_argument(
        ...
        type=int,
        default=0,
        help="step between windows in words (default = window, i.e. non-overlapping)",
    )
    ...
    mb_ext = mb_sub.add_parser(
        "extract",
        help="recover the payload from a watermarked text (needs the ORIGINAL text as reference state)",
    )
    ...
    mb_ext.add_argument(
        "--reference",
        required=True,
        help="the ORIGINAL text file (both parties share the invariant anchor state)",
    )
    mb_ext.add_argument(
        "--reference-stdin",
        action="store_true",
        help="read the original text from stdin instead of a file",
    )
    ...
    ev.add_argument(
        ...
        type=int,
        default=None,
        help="cap the edit budget (default: unlimited until target)",
    )
    ev.add_argument(
        "--ollama-model",
        default=None,
        help="optional local Ollama model for natural candidates per position",
    )
...
def _handle_splash(args: argparse.Namespace) -> int:
...
def _handle_dilute(args: argparse.Namespace) -> int:
    text = _read(args)
...
def _handle_file_inspect(args: argparse.Namespace) -> int:
...
def _handle_file_clean(args: argparse.Namespace) -> int:
...
def _handle_file_embed(args: argparse.Namespace) -> int:
    from .forensics.key_registry import KeyRegistry
...
def _handle_file_detect(args: argparse.Namespace) -> int:
    from .forensics.key_registry import KeyRegistry
...
def _handle_image_score(args: argparse.Namespace) -> int:
...
def _handle_report_sign(args: argparse.Namespace) -> int:
...
def _handle_report_verify(args: argparse.Namespace) -> int:
...
def _handle_report_keygen(args: argparse.Namespace) -> int:
...
def _handle_similarity(args: argparse.Namespace) -> int:
...
def _handle_kgw_sample(args: argparse.Namespace) -> int:
...
def _handle_delta_z(args: argparse.Namespace) -> int:
...
def _handle_payload(args: argparse.Namespace) -> int:
...
def _handle_evade(args: argparse.Namespace) -> int:
...
CMD_HANDLERS: dict[str, callable] = {
    ...
    "clean": _handle_clean,
    "dilute": _handle_dilute,
    "embed": _handle_embed,
    "file-inspect": _handle_file_inspect,
    "file-clean": _handle_file_clean,
    "file-embed": _handle_file_embed,
    "file-detect": _handle_file_detect,
    "rewrite": _handle_rewrite,
    "image-score": _handle_image_score,
    "report": _handle_report,
    "report-sign": _handle_report_sign,
    "report-verify": _handle_report_verify,
    "report-keygen": _handle_report_keygen,
    ...
    "batch": _handle_batch,
    "kgw-sample": _handle_kgw_sample,
    "delta-z": _handle_delta_z,
...
def main() -> int:
    ...
    p.add_argument(
        ...
        "-q",
        action="store_true",
        help="suppress status messages on stderr (machine-readable output only)",
    )
