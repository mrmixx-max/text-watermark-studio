...
def detect(req: DetectRequest, request: Request):
    return respond(request, svc.detect(req.min_size))


@router.post("/summarize")
def summarize(request: Request):
    return respond(request, svc.summarize())


@router.get("/list")
def list_communities(request: Request):
    return respond(request, svc.list())


@router.get("/get")
def get_community(community_id: str, request: Request):
    return respond(request, svc.get(community_id))
