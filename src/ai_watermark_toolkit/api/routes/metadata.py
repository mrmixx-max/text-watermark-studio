...
def embed_file(key_id: str, file: UploadFile = File(...), _auth: None = Depends(require_api_key)):
    if not file.filename:
        raise HTTPException(status_code=400, detail="filename_required")
...
def detect_file(file: UploadFile = File(...), _auth: None = Depends(require_api_key)):
    if not file.filename:
        raise HTTPException(status_code=400, detail="filename_required")
    data = file.file.read()
