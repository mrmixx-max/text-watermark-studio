...
@router.post('/export/zip', summary='Export pipeline output as ZIP', description='Runs pipeline and returns a zip containing text and report.')
def export_zip(req: ExportRequest):
    ...
    return Response(buf.getvalue(), media_type='application/zip', headers={'Content-Disposition': 'attachment; filename="text-watermark-studio-export.zip"'})
