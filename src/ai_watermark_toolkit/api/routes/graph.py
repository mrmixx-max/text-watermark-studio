...
class EdgeRequest(BaseModel): edge: dict
...
@router.get('/query')
def query(label: str): return svc.query(label)
@router.get('/neighbors')
def neighbors(node_id: str): return svc.neighbors(node_id)
@router.get('/subgraph')
def subgraph(seed: str, depth: int = 1): return svc.subgraph(seed, depth)
