...
class ExportService:
    ...
    def _render_html(self, title: str, text: str, metadata: dict, style: str) -> str:
        meta_html = ''.join(f'<li><strong>{html.escape(str(k))}</strong>: {html.escape(str(v))}</li>' for k, v in metadata.items())
    ...
    def _render_csv(self, title: str, text: str, metadata: dict) -> str:
        rows = [['field', 'value'], ['title', title], ['content', text]] + [[str(k), str(v)] for k, v in metadata.items()]
        out = []
        for row in rows:
    ...
    def export(self, title: str, text: str, format: str = 'md', style: str = 'clean', metadata: dict | None = None):
        ...
        return {'path': str(path), 'format': format, 'style': style, 'bytes': path.stat().st_size, 'metadata_keys': list(metadata.keys())}
